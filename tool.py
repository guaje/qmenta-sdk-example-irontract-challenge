...
def run(context):
    ...
    # Experiment setup
    dwi_fp = hcpl_dwi_file_path
    bval_fp = hcpl_bvalues_file_path
    bvec_fp = hcpl_bvecs_file_path

    #################
    # Load the data #
    #################
    #dwi_img = nib.load(hcpl_dwi_file_path)
    dwi_img, dwi_affine = load_nifti(dwi_fp)
    bvals, bvecs = read_bvals_bvecs(bval_fp, bvec_fp)
    gtab = gradient_table(bvals, bvecs)
    #ROIs_img = nib.load(VUMC_ROIs_file_path)
    rois_img, rois_affine = load_nifti(VUMC_ROIs_file_path)

    ############################################
    # Extract the brain mask from the b0 image #
    ############################################
    _, brain_mask = median_otsu(dwi_img, vol_idx=0, median_radius=2, numpass=1)
    ...
    response, ratio = auto_response_ssst(
        gtab, dwi_img.get_data(), roi_radii=10, fa_thr=.7)
    ...
    seed_mask_img, seed_mask_affine = load_nifti(inject_file_path)
    seeds = utils.seeds_from_mask(seed_mask_img, seed_mask_affine, density=5)
    ...
    streamline_generator = LocalTracking(prob_dg, stopping_criterion, seeds,
                                         seed_mask_affine, .2, max_cross=1)
    ...
    # image and apply a small gaussian smoothing. The gaussian smoothing is   #
    # especially useful to increase voxel coverage of deterministic           #
    # algorithms. The log of the smoothed visitation count map is then        #
    # iteratively thresholded producing 200 volumes/operation points. For     #
    # VUMC, additional streamline filtering is done using anatomical priors   #
    # (keeping only streamlines that intersect with at least one ROI).        #
    ###########################################################################
    if postprocessing in ['EPFL', 'ALL']:
        ...
        density = utils.density_map(streamlines, seed_mask_affine,
                                    seed_mask_img.shape)
        density = scipy.ndimage.gaussian_filter(density.astype('float32'), .5)
        ...
        for i, t in enumerate(np.arange(0, max_density, max_density / 200)):
            ...
            vol_filename = os.path.join(volume_folder,
                                        'vol{}_t{}.nii.gz'.format(nbr, t))
            nib.Nifti1Image(mask.astype('int32'), seed_mask_affine,
    ...
    if postprocessing in ['VUMC', 'ALL']:
        ...
        rois = rois_img.astype(np.int)
        _, grouping = utils.connectivity_matrix(streamlines, seed_mask_affine,
                                                rois, inclusive=True,
                                                return_mapping=True,
    ...
    if postprocessing in ['EPFL', 'ALL']:
        context.upload_file(output_epfl_zip_file_path,
                            'KungFuTracking_EPFL_example.zip')
    if postprocessing in ['VUMC', 'ALL']:
        context.upload_file(output_vumc_zip_file_path,
                            'KungFuTracking_VUMC_example.zip')
